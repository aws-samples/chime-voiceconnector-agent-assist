...
function addFederationReferencesToSchema(schema) {
    const doc = graphql_1.parse(common_1.printSchemaWithDirectives(schema));
...
function turnExtensionsIntoObjectTypes(ast) {
    return {
        ...
        definitions: ast.definitions.map(def => {
            ...
            const isDefined = ast.definitions.some(d => d.kind === graphql_1.Kind.OBJECT_TYPE_DEFINITION && d.name.value === def.name.value);
            if (isDefined) {
                return def;
            }
            return {
...
function removeFederation(schema, { withDirectives, }) {
    ...
    const printedSchema = withDirectives ? common_1.printSchemaWithDirectives(schema) : graphql_1.printSchema(schema);
    const astNode = graphql_1.parse(printedSchema);
    const emptyNode = null;
    const docWithoutFederation = graphql_1.visit(astNode, {
    ...
    return graphql_1.buildASTSchema(docWithoutFederation, {
...
class ApolloFederation {
    ...
    transformParentType({ fieldNode, parentType, parentTypeSignature }) {
        if (this.enabled && graphql_1.isObjectType(parentType) && isFederationObjectType(parentType) && fieldNode.name.value === resolveReferenceFieldName) {
